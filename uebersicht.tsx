...
type AnyFeedItem = FeedItem | { kind: 'podcast_banner'; data: PodcastItem; date: Date }
type FeedRow = {
  items: AnyFeedItem[]
  date: Date
}
...
export function ArticlesSection() {
  ...
  const { data: instaData } = useSWR<{ data: InstaPost[] }>('/api/instagram', fetcher)
  const { data: ytData } = useSWR<{ items: YtVideo[] }>('/api/youtube', fetcher)
  const { data: podcastData } = useSWR<{ items: PodcastItem[]; channelImage: string }>('/api/podcast', fetcher)
  const { data: trendingData } = useSWR<{ results: TrendingPage[] }>('/api/trending', fetcher)

  const isLoading = vpLoading || ppLoading

  const vpPosts: WpPost[] = (vpData ?? []).map(p => ({ ...p, source: 'volksverpetzer' as const }))
  const ppPosts: WpPost[] = (ppData ?? []).map(p => ({ ...p, source: 'pruefpunkt' as const }))
  ...
  const recentYt: FeedItem[] = (ytData?.items ?? [])
    .filter(v => v.publishedAt && daysAgo(new Date(v.publishedAt)) <= YT_INTERLEAVE_DAYS)
    .slice(0, 4)
    .map(v => ({ kind: 'youtube', date: new Date(v.publishedAt), data: v }))

  const mergedFeed: FeedItem[] = [...remainingArticles, ...recentInsta, ...recentYt]
  ...
  const latestEpisodeDate = latestEpisode?.pubDate ? new Date(latestEpisode.pubDate) : null
  const podcastIsRecent =
    latestEpisode?.pubDate && daysAgo(new Date(latestEpisode.pubDate)) <= PODCAST_BANNER_DAYS

  const feedRows: FeedRow[] = [...groupedFeedRows]
  if (latestEpisode && latestEpisodeDate && !Number.isNaN(latestEpisodeDate.getTime())) {
    const podcastRow: FeedRow = {
      items: [{ kind: 'podcast_banner', data: latestEpisode, date: latestEpisodeDate }],
      date: latestEpisodeDate,
    }

    if (podcastIsRecent) {
      const insertIdx = feedRows.findIndex(row => row.date.getTime() < latestEpisodeDate.getTime())
      if (insertIdx === -1) {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {isLoading ? (
            ...
            <div>
              {sidebarArticles.map(post => (
                <ArticleCard key={`${post.source}-${post.id}`} post={post} variant="compact" />
              ))}
            </div>
          )}
        </div>
      ...
      {isLoading ? (
        ...
        <FeedGrid items={feedWithPodcast} podcastChannelImage={podcastData?.channelImage ?? ''} />
      )}
    </main>
  )
}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-6">
      {items.map((item, idx) => {
        ...
        if (item.kind === 'podcast_banner') {
          return (
            <div key={`podcast-banner-${idx}`} className={`sm:col-span-2 lg:col-span-3 ${withSeparator}`}>
              <PodcastBanner episode={item.data} channelImage={podcastChannelImage} />
            </div>
          )
        }
        if (item.kind === 'article') {
          return (
            <div key={`article-${item.data.source}-${item.data.id}`} className={withSeparator}>
