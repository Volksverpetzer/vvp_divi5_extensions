...
export const FactCheckSearchApp: React.FC<FactCheckSearchAppProps> = ({ searchApiUrl, importApiUrl }) => {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        if (isUrl(trimmed)) {
            ...
            try {
                const res = await fetch(importApiUrl + '?url=' + encodeURIComponent(trimmed));
                if (!res.ok) throw new Error('HTTP ' + res.status);
                const data = await res.json();
    ...
    const disableSubmit = !query.trim() || phase === 'importing' || phase === 'searching';
            ...
            <section className="vvp-fc__bar">
                <div className="vvp-fc__bar-inner">
                    ...
                    <div className="vvp-fc__bar-actions">
                        <button type="button" className="vvp-fc__bar-trigger js-vvp-fc-open" onClick={openOverlay}>
                            <IconSearch size={15} />
                            <span>z.B. eine strittige Behauptung, ein Zitat oder eine URL...</span>
                        </button>
                        <button type="button" className="vvp-fc__bar-btn js-vvp-fc-open" onClick={openOverlay}>
                            Im Archiv suchen
                        </button>
                    </div>
                </div>
