...
const parseCaption = (text: string): React.ReactNode[] => {
    ...
    return parts.map((part, i) =>
        i % 2 === 1
            ? <a key={i} href={part} target="_blank" rel="noopener noreferrer" style={{ color: '#3893C0', wordBreak: 'break-all' }}>{part}</a>
            : part
    );
};
...
const InternalSlider = ({ slides, activeIndex, setActiveIndex, isCarousel, caption, playingVideos, setPlayingVideos, onCenterClick, fullscreen = false }) => {
    ...
    return (
        ...
        >
            <div className={classnames('vvp-co-slider vvp-co-slider-fullwidth-off', {
                ...
                <div className="vvp-co-slides" style={{ position: 'relative', overflow: 'visible', width: '100%' }}>
                    {slides.map((slide: Slide, index: number) => {
                        ...
                        return (
                            ...
                            >
                                {isVideo ? (
                                    <div className="vvp-co-slide-image" style={fullscreen
                                        ...
                                        {!isPlaying ? (
                                            ...
                                            >
                                                {isNearActive && <img src={slide.thumb || 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs='} alt={caption} style={fullscreen
                                                    ? { height: '100%', width: 'auto', maxWidth: '100%', objectFit: 'contain', display: 'block' }
                                                    : { width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} loading={index === 0 ? 'eager' : 'lazy'} />}
                                            ...
                                            <video loop playsInline preload="metadata" controls autoPlay muted
                                                style={fullscreen
                                                    ? { height: '70vh', width: 'auto', maxWidth: '100%', objectFit: 'contain', display: 'block' }
                                    ...
                                    <div className="vvp-co-slide-image" style={fullscreen
                                        ...
                                        {isNearActive && <img src={slide.thumb} alt={caption} loading={index === 0 ? 'eager' : 'lazy'} decoding="async" style={fullscreen
                                            ? { height: '100%', width: 'auto', maxWidth: '100%', objectFit: 'contain', display: 'block' }
                                            : { width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
                                    </div>
                                )}
                ...
                {caption && <p className="vvp-co__feed-excerpt vvp-co__feed-excerpt--insta" style={{ marginTop: '10px', textAlign: 'left', maxWidth: '100%', padding: '0 .875rem', pointerEvents: 'none', WebkitLineClamp: isCarousel ? 3 : 4 }}>{caption}</p>}
            </div>
        </div>
    );
};

export const InstagramSlideshow: React.FC<InstagramSlideshowProps> = ({ permalink, caption, date, badgeLabel, mediaCategory, slides, isCarousel }) => {
            ...
            <div className="vvp-co__feed-card vvp-co__feed-card--insta">
                ...
                >
                    <div className="vvp-co__feed-footer">
                        <span className="vvp-co__badge vvp-co__badge--insta" style={{ pointerEvents: 'none' }}>
                            <InstaIcon />
                            {badgeLabel}
                        </span>
                        {mediaCategory && <span className="vvp-co__category" style={{ pointerEvents: 'none' }}>{mediaCategory}</span>}
            ...
            {isFullscreen && (
                ...
                >
                    ...
                    >
                        <InternalSlider
                            slides={slides} activeIndex={activeIndex} setActiveIndex={setActiveIndex}
                            isCarousel={isCarousel} caption=""
                            playingVideos={playingVideos} setPlayingVideos={setPlayingVideos}
                            onCenterClick={null} fullscreen={true}
                        />
                        ...
                        <div style={{ background: '#111', padding: '25px 20px', borderRadius: 12, color: 'white', marginTop: 25, marginBottom: 25 }}>
                            ...
                            <p className="vvp-co__insta-fullscreen-caption" style={{ whiteSpace: 'pre-wrap', lineHeight: 1.6, fontSize: 15, margin: 0 }}>{parseCaption(caption)}</p>
